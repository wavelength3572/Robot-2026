...
/**
 * TurretIO implementation for TurretBot using NEO 550 motor with Spark Max and
 * REV Through Bore Encoder for absolute position. Configuration is loaded from the current
 * RobotConfig.
 *
 * NO conversion factors are set on the SparkMax. All encoder readings are raw:
 * Motor encoder: position in motor rotations, velocity in RPM
 * Absolute encoder: position in raw rotations (0.0 to 1.0)
 *
 * All conversions to turret degrees are done in software for full transparency.
 *
 * The absolute encoder sits AFTER the motor gearbox but BEFORE the external
 * gearing to the turret.
 */
public class TurretIOSparkMax implements TurretIO {
    ...
    private final Debouncer motorConnectedDebounce = new Debouncer(0.5, Debouncer.DebounceType.kFalling);
    ...
    public TurretIOSparkMax() {
        ...
        double turretPositionDegrees = motorRotationsToDegrees(adjustedEncoder * config.getTurretMotorGearRatio());
    ...
    public void updateInputs(TurretIOInputs inputs) {
        ...
        ifOk(
                ...
                (value) -> inputs.motorPosition = value);
    ...
    public void setTurretAngle(Rotation2d rotation) {
        ...
        double clampedDegrees = Math.max(minAngleDegrees, Math.min(maxAngleDegrees, rotation.getDegrees()));
