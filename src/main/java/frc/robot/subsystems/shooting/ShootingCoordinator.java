...
/**
 * Orchestrates the shooting system: turret rotation, hood angle, launcher RPM,
 * visualization, and
 * auto-shoot logic. This is the "shooting brain" that coordinates all
 * shot-related subsystems.
 *
 * <p>
 * The turret subsystem only handles physical rotation. ShootingCoordinator
 * decides WHAT to aim
...
public class ShootingCoordinator extends SubsystemBase {
  ...
  private final LoggedTunableNumber trenchHoodMaxDeg = new LoggedTunableNumber("Shots/TrenchMode/HoodMaxDeg", 18.0);
  ...
  private final LoggedTunableNumber passLeftAdjustX = new LoggedTunableNumber("SmartLaunch/Pass/Left/AdjustX", 0.0);
  private final LoggedTunableNumber passLeftAdjustY = new LoggedTunableNumber("SmartLaunch/Pass/Left/AdjustY", 0.0);
  private final LoggedTunableNumber passRightAdjustX = new LoggedTunableNumber("SmartLaunch/Pass/Right/AdjustX", 0.0);
  private final LoggedTunableNumber passRightAdjustY = new LoggedTunableNumber("SmartLaunch/Pass/Right/AdjustY", 0.0);

  // Two-point trajectory tunables for pass shots (dashboard value in inches,
  // converted to meters)
  private final LoggedTunableNumber symmetricArcPeakHeightIn = new LoggedTunableNumber(
      "SmartLaunch/Pass/Symmetric/ArcPeakHeightIn", 98.4);
  private final LoggedTunableNumber lobNetClearanceMarginM = new LoggedTunableNumber(
      "SmartLaunch/Pass/Lob/NetClearanceMarginM", 0.3);
  private final LoggedTunableNumber lobMaxPeakHeightM = new LoggedTunableNumber("SmartLaunch/Pass/Lob/MaxPeakHeightM",
      5.0);
  private final LoggedTunableNumber lobMinHubDistM = new LoggedTunableNumber("SmartLaunch/Pass/Lob/MinHubDistM", 4.0);
  private final LoggedTunableNumber lobStation1AdjustY = new LoggedTunableNumber(
      "SmartLaunch/Pass/DriverStation/Station1/AdjustY", 0.0);
  private final LoggedTunableNumber lobStation3AdjustY = new LoggedTunableNumber(
      "SmartLaunch/Pass/DriverStation/Station3/AdjustY", 0.0);
  ...
  private final LoggedTunableNumber shootOnTheMoveSpeedMps = new LoggedTunableNumber(
      "Shots/SpeedLimits/ShootOnTheMoveSpeedMps", .50);
  // SHOOT_STATIONARY: max speed for hub shots under trench (tight clearance, low
  // hood).
  private final LoggedTunableNumber stationarySpeedMps = new LoggedTunableNumber("Shots/SpeedLimits/StationarySpeedMps",
      0.3);
  // PASS / LONG_PASS: max speed for pass shots in neutral/opponent zones.
  private final LoggedTunableNumber passSpeedMps = new LoggedTunableNumber("Shots/SpeedLimits/PassSpeedMps", 3.0);
  ...
  public ShootingCoordinator(
    ...
    this.turretConfig = new ShotCalculator.TurretConfig(
  ...
  /**
   * Initialize the coordinator with robot pose and speed suppliers. Creates the
   * visualizer and
   ...
   * @param poseSupplier     Supplier for robot's 2D pose
   * @param speedsSupplier   Supplier for field-relative chassis speeds
   * @param pitchDegSupplier Supplier for gyro pitch in degrees (for bump
   *                         detection)
   */
  public void initialize(
    ...
    if (Constants.currentMode == Constants.Mode.SIM) {
      this.visualizer = new ShotVisualizer(
          pose3dSupplier,
          speedsSupplier,
  ...
  private void updateShotCalculation(DriverStation.Alliance alliance, boolean isBlueAlliance) {
    ...
    if (robotPoseSupplier != null
      ...
      TurretAimingHelper.AimResult aimResult = TurretAimingHelper.getAimTarget(
          robotPose.getX(),
          robotPose.getY(),
          alliance,
          ...
          turretFieldPos[0],
          turretFieldPos[1]);
      trenchModeActive = aimResult.zone() == ZoneDetector.Zone.TRENCH_NEAR
      ...
      if (cachedLeftTarget == null
        ...
        double leftRawY = Constants.StrategyConstants.LEFT_PASS_TARGET_Y
            + passLeftAdjustY.get() * (maxY - minY) / 2.0;
        cachedLeftTarget = new Translation3d(
        ...
        double rightRawY = Constants.StrategyConstants.RIGHT_PASS_TARGET_Y
            + passRightAdjustY.get() * (maxY - minY) / 2.0;
        cachedRightTarget = new Translation3d(
      ...
      if (cachedLobStation1Target == null
        ...
        double st12BaseY = isBlueAlliance
        ...
        cachedLobStation1Target = new Translation3d(
        ...
        double st3BaseY = isBlueAlliance
        ...
        cachedLobStation3Target = new Translation3d(
            Math.max(minX, Math.min(maxX, baseX)),
      ...
      switch (aimResult.mode()) {
        ...
        case PASS -> {
          ...
          if (strategy == PassingStrategy.DRIVER_STATION && !isTooCloseToHub(robotPose)) {
            ...
            Translation3d activeTarget = (station <= 2) ? cachedLobStation1Target : cachedLobStation3Target;
            Logger.recordOutput(
        ...
        case LONG_PASS -> {
          ...
          boolean isLeftTrench = selectIsLeftTrench(robotPose);
          Translation3d activeTarget = isLeftTrench ? cachedLobStation1Target : cachedLobStation3Target;
  ...
  private boolean isTooCloseToHub(Pose2d robotPose) {
    ...
    double hubCenterX = isBlue ? FieldConstants.LinesVertical.hubCenter : FieldConstants.LinesVertical.oppHubCenter;
  ...
  /**
   * Select whether to pass to the left trench based on the active passing
   * strategy.
   *
   * <p>
   * SYMMETRIC: picks based on robot Y position relative to field center.
   * DRIVER_STATION: picks
   * based on FMS driver station number (1=left, 3=right, 2=fallback to
   * symmetric).
   */
  ...
  private final LoggedTunableNumber passMaxRPM = new LoggedTunableNumber("SmartLaunch/Pass/MaxRPM", 4000.0);
  ...
  private void calculateShotToHub(
    ...
    Translation3d hubTarget = isBlueAlliance
  ...
  private void calculateShotToTarget(
    ...
    ShotCalculator.ShotResult activeResult = activeStrategy.calculateShot(
        robotPose,
        fieldSpeeds,
        target,
        turretConfig,
    ...
    if (periodicCounter % 5 == 0) {
      ...
      double distanceToTarget = Math.sqrt(Math.pow(target.getX() - turretX, 2) + Math.pow(target.getY() - turretY, 2));
  ...
  private void calculatePassToTarget(
    ...
    double horizontalDist = Math.sqrt(Math.pow(target.getX() - turretX, 2) + Math.pow(target.getY() - turretY, 2));
    ...
    if (strategy == PassingStrategy.DRIVER_STATION) {
      ...
      double hubDistAlongShot = (hubCenterX - turretX) * shotDirX + (hubCenterY - turretY) * shotDirY;
      ...
      constraintX = horizontalDist / 2.0;
      constraintH = symmetricArcPeakHeightIn.get() * 0.0254;
      maxPeakHeight = symmetricArcPeakHeightIn.get() * 0.0254 + 1.0; // allow small margin above desired peak
    }

    ShotCalculator.ShotResult result = ShotCalculator.calculatePassShotTwoPoint(
        robotPose,
        fieldSpeeds,
        target,
        turretConfig,
  ...
  /**
   * Log current shot state to AdvantageKit every cycle. Runs in periodic() so
   * readiness, targets,
   * and tracking errors are always visible — regardless of which command (smart
   * launch, fixed shot,
  ...
  private void logShotState() {
    ...
    boolean motivatorReady = motivator == null || motivator.getState() == Motivator.MotivatorState.READY;
    ...
    if (currentShot != null && robotPoseSupplier != null) {
      Pose2d robotPose = robotPoseSupplier.get();
      double robotHeadingRad = robotPose.getRotation().getRadians();
      double[] turretPos = ShotCalculator.getTurretFieldPosition(
          robotPose.getX(), robotPose.getY(), robotHeadingRad, turretConfig);
      ...
      double parametricTOF = ShotCalculator.calculateTimeOfFlight(
  ...
  public void launchFuel() {
    ...
    if (visualizer != null && currentShot != null && robotPoseSupplier != null) {
      ...
      double actualExitVelocity = (denom > 0 && distanceToTarget >= 0.1)
  ...
  public Command repeatedlyLaunchFuelCommand() {
    if (visualizer == null) {
      return runOnce(() -> {
      }); // No-op if visualizer not initialized
    }
  ...
  public void setManualShotParameters(
    ...
    if (robotPoseSupplier != null) {
      Pose2d robotPose = robotPoseSupplier.get();
      currentShot = ShotCalculator.calculateManualShot(
  ...
  /**
   * Check if the robot is slow enough to fire in the current zone. Uses
   * zone-aware speed
   * thresholds: alliance zone allows shoot-on-the-move, trench requires
   * stationary, pass zones
  ...
  public boolean isRobotSlowEnoughForCurrentZone() {
    if (fieldSpeedsSupplier == null || robotPoseSupplier == null)
      return true;
    ...
    double thresholdMps = switch (aimResult.mode()) {
    ...
    boolean slowEnough = aimResult.mode() != TurretAimingHelper.AimMode.NONE && robotSpeedMps <= thresholdMps;
  ...
  /**
   * Check if the robot is currently in a pass zone (PASS or LONG_PASS aim mode).
   * Used by
   * continuousSmartLaunch to arm feeding only after reaching the pass zone during
   * sprint autos.
   *
  ...
  public ZoneDetector.Zone getCurrentZone() {
    if (robotPoseSupplier == null)
      return ZoneDetector.Zone.ALLIANCE;
    Pose2d robotPose = robotPoseSupplier.get();
    DriverStation.Alliance alliance = RobotStatus.getAlliance();
  ...
  /**
   * Check if the robot is stationary (speed <= stationarySpeedMps tunable). Used
   * by auto-tracking
  ...
  /**
   * Check if trench avoidance is currently active (robot is in a trench zone).
   * Trench clamping is
   * always enabled — there is no toggle. The hood is clamped whenever the robot
   * is detected inside
   * any trench zone.
   */
  ...
  /**
   * Reload LUT data. Loads the hardcoded baseline table first, then overlays any
   * field-recorded
   * entries on top (field data overrides baseline at matching distances).
   *
   * <p>
   * To update the baseline: edit {@link ShotTableConstants#BASELINE_TABLE} and
   * push code. To add
  ...
  /**
   * Get the horizontal distance from the turret to the current aim target.
   * Updated every cycle by
  ...
  /**
   * Get the zone-appropriate drive speed limit. In ALLIANCE zone returns just
   * under the
   * shoot-on-the-move threshold; in PASS/LONG_PASS zones returns the pass
   * threshold. Returns max
  ...
  private ShotSnapshot createVisualizerSnapshot(boolean isBlueAlliance) {
    ...
    if (currentShot == null) {
      ...
      readiness = (launcherReady && motivatorReady && turretReady && hoodReady && achievable)
