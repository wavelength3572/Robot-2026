...
public class ShootingCommands {
  ...
  // Tune up from dashboard to find the threshold where ball delivery becomes inconsistent.
  // Watch Launcher/BallImpact/DipRPM — consistent dips = good, random dips = too fast.
  private static final LoggedTunableNumber motivatorLauncherRatio =
      new LoggedTunableNumber(
          "Shots/SmartLaunch/MotivatorLauncherRatio", 0.2); // tune up, watch BallImpact logs

  // Spindexer RPM lerped by distance: close = max, far = min
  // Tune up from dashboard while watching BallImpact logs for consistency.
