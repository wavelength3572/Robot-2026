...
/**
 * Factory class for shooting commands. Provides a unified launch command that
 * works for both
...
public class ShootingCommands {
  ...
  private static final LoggedTunableNumber hubShotLauncherRPM = new LoggedTunableNumber("Shots/HubShot/LauncherRPM",
      2450.0);
  private static final LoggedTunableNumber hubShotHoodAngleDeg = new LoggedTunableNumber("Shots/HubShot/HoodAngleDeg",
      15.0);
  private static final LoggedTunableNumber hubShotTurretAngleDeg = new LoggedTunableNumber(
      "Shots/HubShot/TurretAngleDeg", -90);
  private static final LoggedTunableNumber hubShotMotivatorRPM = new LoggedTunableNumber("Shots/HubShot/MotivatorRPM",
      1300.0);
  private static final LoggedTunableNumber hubShotSpindexerRPM = new LoggedTunableNumber("Shots/HubShot/SpindexerRPM",
      325.0);

  // Left trench shot
  private static final LoggedTunableNumber leftTrenchLauncherRPM = new LoggedTunableNumber(
      "Shots/LeftTrench/LauncherRPM", 2650.0);
  private static final LoggedTunableNumber leftTrenchHoodAngleDeg = new LoggedTunableNumber(
      "Shots/LeftTrench/HoodAngleDeg", 18.0);
  private static final LoggedTunableNumber leftTrenchTurretAngleDeg = new LoggedTunableNumber(
      "Shots/LeftTrench/TurretAngleDeg", 186.5);
  private static final LoggedTunableNumber leftTrenchMotivatorRPM = new LoggedTunableNumber(
      "Shots/LeftTrench/MotivatorRPM", 1800.0);
  private static final LoggedTunableNumber leftTrenchSpindexerRPM = new LoggedTunableNumber(
      "Shots/LeftTrench/SpindexerRPM", 325.0);

  // Right trench shot
  private static final LoggedTunableNumber rightTrenchLauncherRPM = new LoggedTunableNumber(
      "Shots/RightTrench/LauncherRPM", 3169.0);
  private static final LoggedTunableNumber rightTrenchHoodAngleDeg = new LoggedTunableNumber(
      "Shots/RightTrench/HoodAngleDeg", 18.0);
  private static final LoggedTunableNumber rightTrenchTurretAngleDeg = new LoggedTunableNumber(
      "Shots/RightTrench/TurretAngleDeg", -4.84);
  private static final LoggedTunableNumber rightTrenchMotivatorRPM = new LoggedTunableNumber(
      "Shots/RightTrench/MotivatorRPM", 1800.0);
  private static final LoggedTunableNumber rightTrenchSpindexerRPM = new LoggedTunableNumber(
      "Shots/RightTrench/SpindexerRPM", 325.0);

  // ===== Robot Tuning (affects real robot behavior) =====

  // Motivator RPM as a ratio of launcher RPM: motivatorRPM = launcherRPM * ratio
  private static final LoggedTunableNumber motivatorLauncherRatio = new LoggedTunableNumber(
      "Shots/SmartLaunch/MotivatorLauncherRatio", 0.565);

  // Spindexer RPM lerped by distance: close = max, far = min
  private static final LoggedTunableNumber spindexerCloseRPM = new LoggedTunableNumber(
      "Shots/SmartLaunch/SpindexerCloseRPM", 325.0);
  private static final LoggedTunableNumber spindexerFarRPM = new LoggedTunableNumber(
      "Shots/SmartLaunch/SpindexerFarRPM", 325.0);
  // Fixed spindexer RPM used in pass/neutral zones (no distance lerp)
  private static final LoggedTunableNumber spindexerPassRPM = new LoggedTunableNumber(
      "Shots/SmartLaunch/SpindexerPassRPM", 350.0);

  // ===== LUT Dev Overrides (manual RPM/hood for data collection) =====
  private static final LoggedTunableNumber lutDevOverrideRPM = new LoggedTunableNumber("LUTDev/OverrideRPM", 2500.0);
  private static final LoggedTunableNumber lutDevOverrideHoodDeg = new LoggedTunableNumber("LUTDev/OverrideHoodDeg",
      25.0);
  ...
  /**
   * Set the launcher RPM trim offset. This value is added to all launcher RPM
   * targets (both smart
  ...
  /**
   * Fixed-position launch command. Moves all subsystems to tunable setpoints in
   * parallel, waits for
   * ready (with 5s timeout), then feeds via spindexer. All parameters are
   * suppliers so dashboard
   ...
   * @param turretAngleDegSupplier Supplier for target outside turret angle in
   *                               degrees
            ...
            Commands.sequence(
                Commands.waitUntil(
                    () -> {
                      ...
                      boolean motivatorReady = motivator == null
  ...
  /**
   * Smart launch command. Continuously reads shot parameters from the
   * ShootingCoordinator's
   * odometry-based calculations (hub or pass depending on field position) and
   * commands all
  ...
  /**
   * Core smart launch Sequenced. Uses zone-aware speed gating for both readiness
   * checks and
   * spindexer feeding.
   */
            ...
            Commands.sequence(
                Commands.waitUntil(
                    () -> {
                      ...
                      boolean turretReady = turret.getState() == Turret.TurretState.READY;
                      boolean hoodReady = hood == null || hood.getState() == Hood.HoodState.READY;
                      boolean achievable = shot != null && (isLutDevOverrideActive() || shot.achievable());
                      // boolean robotSlow = coordinator.isRobotSlowEnoughForCurrentZone();

                      boolean allReady = launcherReady && turretReady && hoodReady && achievable;
                ...
                ? Commands.run(
                    new Runnable() {
                      ...
                      public void run() {
                        ...
                        double spnRPM = coordinator.isInPassZone()
                            ? getSpindexerPassRPM()
  ...
  /**
   * Core smart launch implementation. Uses zone-aware speed gating for both
            ...
            Commands.sequence(
                Commands.waitUntil(
                    () -> {
                      ...
                      boolean allReady = launcherReady
                          && motivatorReady
                          && turretReady
                          && hoodReady
                          && achievable
  ...
  /**
   * Smart launch with drive speed limiting. Sets a global speed limit on the
   * default drive command
   * while active. When released, the speed limit ramps back up smoothly (via
   * {@link
   * DriveCommands#clearSpeedLimit()}) to prevent sudden acceleration if the
   * driver is pushing the
   * stick forward.
   *
   * @param launcher    The launcher subsystem
   * @param coordinator The shooting coordinator
  ...
  /**
   * Auto-tracking command. Continuously aims turret and hood at the calculated
   * target based on
   * odometry. When used as a toggle, keeps the turret/hood pre-aimed so that
   * firing commands
   * (smartLaunch, hub shot, etc.) can skip the positioning phase — they interrupt
   * this command via
  ...
  /**
   * Check if all shooting subsystems are ready to fire. Uses state machines
   * rather than raw
  ...
  /**
   * Continuous smart launch command for use during autonomous paths. Unlike
   * smartLaunchCommand,
   * this has no initial wait phase — it starts tracking and firing immediately
   * (launcher may
   * already be spinning from initialSmartLaunch). Runs until interrupted (path
   * ends → parallel
   * group ends).
   *
   * <p>
   * Uses zone-based speed gating from the coordinator for hub vs pass speed
   * limits. Controls all
   * shooting subsystems (launcher RPM, turret angle, hood angle, motivator,
   * spindexer) from live
   * shot calculations.
   *
   * @param launcher      The launcher subsystem
   * @param coordinator   The shooting coordinator (provides shot calculations and
   *                      zone checks)
   * @param motivator     The motivator subsystem (can be null)
   * @param turret        The turret subsystem
   * @param hood          The hood subsystem (can be null)
   * @param spindexer     The spindexer subsystem (can be null)
   * @param armOnPassZone When true, feeding is suppressed until the robot first
   *                      enters a PASS or
   *                      LONG_PASS zone. Used by SPRINT autos to prevent firing
   *                      preloads at the start position.
   *                      Launcher/turret/hood still track the live shot while
   *                      waiting.
  ...
  public static Command continuousSmartLaunchCommand(
    ...
    // Once armed, stays armed permanently (robot can return to alliance zone and
    // fire).
    // wasOutsidePassZone: prevents arming if the robot starts in/near the pass
    // zone.
    // Must leave the pass zone first, then re-enter to arm. For SPRINT autos
    // starting
    // in the alliance zone this is immediate; for edge cases it prevents false
    // arming.
    ...
    return Commands.parallel(
        ...
        // Spindexer — feed fuel (gated on armed + launcher ready + turret aimed + zone
        // speed,
        // reciprocate in neutral zone only when launcher is at speed, hold still
        // otherwise)
        spindexer != null
            ? Commands.run(
                new Runnable() {
  ...
  /**
   * Auto-tracking stationary shooting command. Continuously tracks the hub
   * (turret, launcher,
   * motivator) in far trench, near trench, and alliance zones, but only feeds the
   * spindexer when
   * the robot is stationary in near trench or alliance zone. Hood tracks target
   * angle in near
   * trench and alliance zone only — in far trench the hood stays at min angle for
   * safety.
   *
   * <p>
   * Designed for autos where the path stops in a shooting zone to fire
   ...
   * @param hood        The hood subsystem (can be null)
   * @param spindexer   The spindexer subsystem (can be null)
   * @return Command that tracks continuously and fires when stationary in a safe
   *         zone
   */
    ...
    return Commands.parallel(
        ...
        Commands.run(
            () -> {
              ...
              boolean inTrackZone = zone == ZoneDetector.Zone.TRENCH_FAR
        ...
        spindexer != null
            ? Commands.run(
                () -> {
                  ZoneDetector.Zone zone = coordinator.getCurrentZone();
                  boolean inShootZone = zone == ZoneDetector.Zone.TRENCH_NEAR
                      || zone == ZoneDetector.Zone.ALLIANCE;
  ...
  /**
   * Sim firing loop for auto-tracking stationary. Gates on stationary + in shoot
   * zone (near trench
   * or alliance).
   */
    ...
    return Commands.sequence(
        Commands.waitUntil(
            () -> {
              ...
              boolean stationary = coordinator.isRobotStationary();
              ZoneDetector.Zone zone = coordinator.getCurrentZone();
              boolean inShootZone = zone == ZoneDetector.Zone.TRENCH_NEAR || zone == ZoneDetector.Zone.ALLIANCE;
              return subsReady && stationary && inShootZone;
            }),
        Commands.runOnce(
            () -> {
              ShotVisualizer visualizer = coordinator.getVisualizer();
              if (visualizer != null && visualizer.getFuelCount() <= 0)
                return;
  ...
  /**
   * Sim firing loop for continuous smart launch. Uses isAllSubsystemsReady + zone
   * speed gating +
   * feeding armed check.
   */
  ...
  /**
   * Simulation firing loop. Waits for alignment and setpoint, then launches fuel
   * repeatedly.
   *
   * @param coordinator The shooting coordinator
   * @param launcher    The launcher subsystem
  ...
  /**
   * Seeds the LUT dev override sliders from the current calculated shot, so you
   * can start from the
  ...
  private static double calculateDistanceToHub(
    ...
    double[] turretFieldPos = ShotCalculator.getTurretFieldPosition(
  ...
  /**
   * End a batch and record LUT data. Call after firing a hopper of fuel from a
   * stationary position.
   * Press success if the shots scored, miss if they didn't. Writes clean LUT data
   * (success only)
    ...
    return Commands.runOnce(
        () -> {
          ...
          if (currentShot != null && currentShot.exitVelocityMps() > 0) {
            theoreticalTOF = ShotCalculator.calculateTimeOfFlight(
          ...
          int fuelFired = recorder.endBatch(
          ...
          String summary = (successful ? "HIT" : "MISS")
              + " | "
  ...
  /**
   * LUT dev mode logging command. Runs continuously to output useful info to the
   * dashboard while
   * collecting LUT data.
   *
   * @param coordinator The shooting coordinator
